...
const int
   ...
   et_sentarse	 = 2 ,
   et_levantarse = 3 ;
//Booleano para identificar el modo en el que se encuentra el camarero
    bool modo1=true;
...
void funcion_camarero(int id)
{
    ...
    {
	//Examen
	//Siempre que haya 0 filosofos sentados, se activará el modo 1
	if(filo_sentados==0){
	   modo1=true;	
	}
	//Si estoy en el modo 1 pero hay cuatro filosofos sentados, activo modo 2= !modo1
	else if (modo1 and filo_sentados==4)
	  modo1=false;
	//si esta activo el modo 1 solo dejo que se sienten (cambio la etiqueta)
        if(modo1 and filo_sentados<4)
	   MPI_Probe(MPI_ANY_SOURCE, et_sentarse, MPI_COMM_WORLD, &estado); // Puede sentarse
	//Si estoy en modo 2 el camarero se comporta de forma permisiva
	else if (modo1==false and filo_sentados < 4) // El maximo de filosofos comiendo es 4
